...
#Adopted from https://github.com/joshuaskelly/earcut-python
...
def earcutLinked(ear, triangles, dim, minX, minY, size, _pass=None):
    ...
    while ear.prev != ear.next:
        ...
        if ear == stop:
            ...
            if not _pass:
                earcutLinked(filterPoints(ear), triangles,
                             dim, minX, minY, size, 1)
...
def isEarHashed(ear, minX, minY, size):
    ...
    minTX = (a.x if a.x < c.x else c.x) if a.x < b.x else (
        b.x if b.x < c.x else c.x)
    minTY = (a.y if a.y < c.y else c.y) if a.y < b.y else (
        b.y if b.y < c.y else c.y)
    maxTX = (a.x if a.x > c.x else c.x) if a.x > b.x else (
        b.x if b.x > c.x else c.x)
    maxTY = (a.y if a.y > c.y else c.y) if a.y > b.y else (
        b.y if b.y > c.y else c.y)
